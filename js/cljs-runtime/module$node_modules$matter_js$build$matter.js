shadow$provide.module$node_modules$matter_js$build$matter=function(global,require,module$jscomp$1,exports$jscomp$0){(function(root,factory){"object"===typeof exports$jscomp$0&&"object"===typeof module$jscomp$1?module$jscomp$1.exports=factory():"function"===typeof define&&define.amd?define("Matter",[],factory):"object"===typeof exports$jscomp$0?exports$jscomp$0.Matter=factory():root.Matter=factory()})(this,function(){return function(modules){function __webpack_require__(moduleId){if(installedModules[moduleId])return installedModules[moduleId].exports;
...
height,options){options=options||{};x={label:"Rectangle Body",position:{x,y},vertices:Vertices.fromPath("L 0 0 L "+width+" 0 L "+width+" "+height+" L 0 "+height)};options.chamfer&&(y=options.chamfer,x.vertices=Vertices.chamfer(x.vertices,y.radius,y.quality,y.qualityMin,y.qualityMax),delete options.chamfer);return Body.create(Common.extend({},x,options))};Bodies.trapezoid=function(x,y,width,height,slope,options){options=options||{};slope*=.5;var x1=width*slope;width=x1+(1-2*slope)*width;var x3=width+
x1;x={label:"Trapezoid Body",position:{x,y},vertices:Vertices.fromPath(.5>slope?"L 0 0 L "+x1+" "+-height+" L "+width+" "+-height+" L "+x3+" 0":"L 0 0 L "+width+" "+-height+" L "+x3+" 0")};options.chamfer&&(y=options.chamfer,x.vertices=Vertices.chamfer(x.vertices,y.radius,y.quality,y.qualityMin,y.qualityMax),delete options.chamfer);return Body.create(Common.extend({},x,options))};Bodies.circle=function(x,y,radius,options,maxSides){options=options||{};var circle={label:"Circle Body",circleRadius:radius};
...
padding||{x:0,y:0};for(var JSCompiler_object_inline_x_2225=Infinity,JSCompiler_object_inline_y_2226=Infinity,JSCompiler_object_inline_x_2227=-Infinity,JSCompiler_object_inline_y_2228=-Infinity,i=0;i<objects.length;i+=1){var object=objects[i],min=object.bounds?object.bounds.min:object.min||object.position||object;object=object.bounds?object.bounds.max:object.max||object.position||object;min&&object&&(min.x<JSCompiler_object_inline_x_2225&&(JSCompiler_object_inline_x_2225=min.x),object.x>JSCompiler_object_inline_x_2227&&
(JSCompiler_object_inline_x_2227=object.x),min.y<JSCompiler_object_inline_y_2226&&(JSCompiler_object_inline_y_2226=min.y),object.y>JSCompiler_object_inline_y_2228&&(JSCompiler_object_inline_y_2228=object.y))}objects=JSCompiler_object_inline_x_2227-JSCompiler_object_inline_x_2225+2*padding.x;JSCompiler_object_inline_y_2228=JSCompiler_object_inline_y_2228-JSCompiler_object_inline_y_2226+2*padding.y;JSCompiler_object_inline_x_2227=render.canvas.width/render.canvas.height;i=objects/JSCompiler_object_inline_y_2228;
object=min=1;i>JSCompiler_object_inline_x_2227?object=i/JSCompiler_object_inline_x_2227:min=JSCompiler_object_inline_x_2227/i;render.options.hasBounds=!0;render.bounds.min.x=JSCompiler_object_inline_x_2225;render.bounds.max.x=JSCompiler_object_inline_x_2225+objects*min;render.bounds.min.y=JSCompiler_object_inline_y_2226;render.bounds.max.y=JSCompiler_object_inline_y_2226+JSCompiler_object_inline_y_2228*object;center&&(render.bounds.min.x+=.5*objects-objects*min*.5,render.bounds.max.x+=.5*objects-
objects*min*.5,render.bounds.min.y+=.5*JSCompiler_object_inline_y_2228-JSCompiler_object_inline_y_2228*object*.5,render.bounds.max.y+=.5*JSCompiler_object_inline_y_2228-JSCompiler_object_inline_y_2228*object*.5);render.bounds.min.x-=padding.x;render.bounds.max.x-=padding.x;render.bounds.min.y-=padding.y;render.bounds.max.y-=padding.y;render.mouse&&(Mouse.setScale(render.mouse,{x:(render.bounds.max.x-render.bounds.min.x)/render.canvas.width,y:(render.bounds.max.y-render.bounds.min.y)/render.canvas.height}),
Mouse.setOffset(render.mouse,render.bounds.min))};Render.startViewTransform=function(render){render.context.setTransform(render.options.pixelRatio/((render.bounds.max.x-render.bounds.min.x)/render.options.width),0,0,render.options.pixelRatio/((render.bounds.max.y-render.bounds.min.y)/render.options.height),0,0);render.context.translate(-render.bounds.min.x,-render.bounds.min.y)};Render.endViewTransform=function(render){render.context.setTransform(render.options.pixelRatio,0,0,render.options.pixelRatio,
0,0)};Render.world=function(render,time){time=Common.now();var engine=render.engine,world=engine.world,canvas=render.canvas,context=render.context,options=render.options,timing=render.timing,allBodies=Composite.allBodies(world);world=Composite.allConstraints(world);var background=options.wireframes?options.wireframeBackground:options.background,bodies=[],constraints=[],event={timestamp:engine.timing.timestamp};Events.trigger(render,"beforeRender",event);if(render.currentBackground!==background){var cssBackground=
background;/(jpg|gif|png)$/.test(background)&&(cssBackground="url("+background+")");render.canvas.style.background=cssBackground;render.canvas.style.backgroundSize="contain";render.currentBackground=background}context.globalCompositeOperation="source-in";context.fillStyle="transparent";context.fillRect(0,0,canvas.width,canvas.height);context.globalCompositeOperation="source-over";if(options.hasBounds){for(canvas=0;canvas<allBodies.length;canvas++)background=allBodies[canvas],Bounds.overlaps(background.bounds,
render.bounds)&&bodies.push(background);for(canvas=0;canvas<world.length;canvas++){allBodies=world[canvas];background=allBodies.bodyA;cssBackground=allBodies.bodyB;var pointAWorld=allBodies.pointA,pointBWorld=allBodies.pointB;background&&(pointAWorld=Vector.add(background.position,allBodies.pointA));cssBackground&&(pointBWorld=Vector.add(cssBackground.position,allBodies.pointB));pointAWorld&&pointBWorld&&(Bounds.contains(render.bounds,pointAWorld)||Bounds.contains(render.bounds,pointBWorld))&&constraints.push(allBodies)}Render.startViewTransform(render);
...
endCol,startRow,endRow){return{id:startCol+","+endCol+","+startRow+","+endRow,startCol,endCol,startRow,endRow}};Grid._getBucketId=function(column,row){return"C"+column+"R"+row};Grid._createBucket=function(buckets,bucketId){return buckets[bucketId]=[]};Grid._bucketAddBody=function(grid,bucket,body){for(var i=0;i<bucket.length;i++){var bodyB=bucket[i];if(!(body.id===bodyB.id||body.isStatic&&bodyB.isStatic)){var pairId=Pair.id(body,bodyB),pair=grid.pairs[pairId];pair?pair[2]+=1:grid.pairs[pairId]=[body,
bodyB,1]}}bucket.push(body)};Grid._bucketRemoveBody=function(grid,bucket,body){bucket.splice(Common.indexOf(bucket,body),1);for(var i=0;i<bucket.length;i++){var pairId=Pair.id(body,bucket[i]);(pairId=grid.pairs[pairId])&&--pairId[2]}};Grid._createActivePairsList=function(grid){var pairs=[];var pairKeys=Common.keys(grid.pairs);for(var k=0;k<pairKeys.length;k++){var pair=grid.pairs[pairKeys[k]];0<pair[2]?pairs.push(pair):delete grid.pairs[pairKeys[k]]}return pairs}})()},function(module,exports,__webpack_require__){module=
module.exports=__webpack_require__(23);module.Axes=__webpack_require__(10);module.Bodies=__webpack_require__(11);module.Body=__webpack_require__(6);module.Bounds=__webpack_require__(1);module.Common=__webpack_require__(0);module.Composite=__webpack_require__(5);module.Composites=__webpack_require__(24);module.Constraint=__webpack_require__(8);module.Contact=__webpack_require__(17);module.Detector=__webpack_require__(13);module.Engine=__webpack_require__(18);module.Events=__webpack_require__(4);module.Grid=
__webpack_require__(21);module.Mouse=__webpack_require__(12);module.MouseConstraint=__webpack_require__(25);module.Pair=__webpack_require__(9);module.Pairs=__webpack_require__(20);module.Plugin=__webpack_require__(15);module.Query=__webpack_require__(26);module.Render=__webpack_require__(16);module.Resolver=__webpack_require__(19);module.Runner=__webpack_require__(27);module.SAT=__webpack_require__(14);module.Sleeping=__webpack_require__(7);module.Svg=__webpack_require__(28);module.Vector=__webpack_require__(2);
module.Vertices=__webpack_require__(3);module.World=__webpack_require__(29);module.Engine.run=module.Runner.run;module.Common.deprecated(module.Engine,"run","Engine.run ➤ use Matter.Runner.run(engine) instead")},function(module,exports,__webpack_require__){var Matter={};module.exports=Matter;var Plugin=__webpack_require__(15),Common=__webpack_require__(0);(function(){Matter.name="matter-js";Matter.version="0.17.1";Matter.uses=[];Matter.used=[];Matter.use=function(){Plugin.use(Matter,Array.prototype.slice.call(arguments))};
Matter.before=function(path,func){path=path.replace(/^Matter./,"");return Common.chainPathBefore(Matter,path,func)};Matter.after=function(path,func){path=path.replace(/^Matter./,"");return Common.chainPathAfter(Matter,path,func)}})()},function(module,exports,__webpack_require__){var Composites={};module.exports=Composites;var Composite=__webpack_require__(5),Constraint=__webpack_require__(8),Common=__webpack_require__(0),Body=__webpack_require__(6),Bodies=__webpack_require__(11),deprecated=Common.deprecated;
(function(){Composites.stack=function(xx,yy,columns,rows,columnGap,rowGap,callback){for(var stack=Composite.create({label:"Stack"}),x=xx,lastBody,i=0,row=0;row<rows;row++){for(var maxHeight=0,column=0;column<columns;column++){var body=callback(x,yy,column,row,lastBody,i);body?(x=body.bounds.max.y-body.bounds.min.y,lastBody=body.bounds.max.x-body.bounds.min.x,x>maxHeight&&(maxHeight=x),Body.translate(body,{x:.5*lastBody,y:.5*x}),x=body.bounds.max.x+columnGap,Composite.addBody(stack,body),lastBody=
body,i+=1):x+=columnGap}yy+=maxHeight+rowGap;x=xx}return stack};Composites.chain=function(composite,xOffsetA,yOffsetA,xOffsetB,yOffsetB,options){for(var bodies=composite.bodies,i=1;i<bodies.length;i++){var bodyA=bodies[i-1],bodyB=bodies[i];bodyA=Common.extend({bodyA,pointA:{x:(bodyA.bounds.max.x-bodyA.bounds.min.x)*xOffsetA,y:(bodyA.bounds.max.y-bodyA.bounds.min.y)*yOffsetA},bodyB,pointB:{x:(bodyB.bounds.max.x-bodyB.bounds.min.x)*xOffsetB,y:(bodyB.bounds.max.y-bodyB.bounds.min.y)*yOffsetB}},options);
Composite.addConstraint(composite,Constraint.create(bodyA))}composite.label+=" Chain";return composite};Composites.mesh=function(composite,columns,rows,crossBrace,options){var bodies=composite.bodies,row,col;for(row=0;row<rows;row++){for(col=1;col<columns;col++){var bodyA=bodies[col-1+row*columns];var bodyB=bodies[col+row*columns];Composite.addConstraint(composite,Constraint.create(Common.extend({bodyA,bodyB},options)))}if(0<row)for(col=0;col<columns;col++)bodyA=bodies[col+(row-1)*columns],bodyB=
bodies[col+row*columns],Composite.addConstraint(composite,Constraint.create(Common.extend({bodyA,bodyB},options))),crossBrace&&0<col&&(bodyA=bodies[col-1+(row-1)*columns],Composite.addConstraint(composite,Constraint.create(Common.extend({bodyA,bodyB},options)))),crossBrace&&col<columns-1&&(bodyA=bodies[col+1+(row-1)*columns],Composite.addConstraint(composite,Constraint.create(Common.extend({bodyA,bodyB},options))))}composite.label+=" Mesh";return composite};Composites.pyramid=function(xx,yy,columns,
rows,columnGap,rowGap,callback){return Composites.stack(xx,yy,columns,rows,columnGap,rowGap,function(x,y,column,row,lastBody,i){var actualRows=Math.min(rows,Math.ceil(columns/2));x=lastBody?lastBody.bounds.max.x-lastBody.bounds.min.x:0;if(!(row>actualRows||(row=actualRows-row,actualRows=columns-1-row,column<row||column>actualRows)))return 1===i&&Body.translate(lastBody,{x:(column+(1===columns%2?1:-1))*x,y:0}),callback(xx+(lastBody?column*x:0)+column*columnGap,y,column,row,lastBody,i)})};Composites.newtonsCradle=
function(xx,yy,number,size,length){for(var newtonsCradle=Composite.create({label:"Newtons Cradle"}),i=0;i<number;i++){var circle=Bodies.circle(xx+1.9*i*size,yy+length,size,{inertia:Infinity,restitution:1,friction:0,frictionAir:1E-4,slop:1}),constraint=Constraint.create({pointA:{x:xx+1.9*i*size,y:yy},bodyB:circle});Composite.addBody(newtonsCradle,circle);Composite.addConstraint(newtonsCradle,constraint)}return newtonsCradle};deprecated(Composites,"newtonsCradle","Composites.newtonsCradle ➤ moved to newtonsCradle example");
Composites.car=function(xx,yy,width,height,wheelSize){var group=Body.nextGroup(!0),wheelAOffset=.5*-width+20,wheelBOffset=.5*width-20,car=Composite.create({label:"Car"});width=Bodies.rectangle(xx,yy,width,height,{collisionFilter:{group},chamfer:{radius:.5*height},density:2E-4});height=Bodies.circle(xx+wheelAOffset,yy+0,wheelSize,{collisionFilter:{group},friction:.8});xx=Bodies.circle(xx+wheelBOffset,yy+0,wheelSize,{collisionFilter:{group},friction:.8});wheelAOffset=Constraint.create({bodyB:width,
pointB:{x:wheelAOffset,y:0},bodyA:height,stiffness:1,length:0});wheelBOffset=Constraint.create({bodyB:width,pointB:{x:wheelBOffset,y:0},bodyA:xx,stiffness:1,length:0});Composite.addBody(car,width);Composite.addBody(car,height);Composite.addBody(car,xx);Composite.addConstraint(car,wheelAOffset);Composite.addConstraint(car,wheelBOffset);return car};deprecated(Composites,"car","Composites.car ➤ moved to car example");Composites.softBody=function(xx,yy,columns,rows,columnGap,rowGap,crossBrace,particleRadius,
particleOptions,constraintOptions){particleOptions=Common.extend({inertia:Infinity},particleOptions);constraintOptions=Common.extend({stiffness:.2,render:{type:"line",anchors:!1}},constraintOptions);xx=Composites.stack(xx,yy,columns,rows,columnGap,rowGap,function(x,y){return Bodies.circle(x,y,particleRadius,particleOptions)});Composites.mesh(xx,columns,rows,crossBrace,constraintOptions);xx.label="Soft Body";return xx};deprecated(Composites,"softBody","Composites.softBody ➤ moved to softBody and cloth examples")})()},
function(module,exports,__webpack_require__){var MouseConstraint={};module.exports=MouseConstraint;var Vertices=__webpack_require__(3),Sleeping=__webpack_require__(7),Mouse=__webpack_require__(12),Events=__webpack_require__(4),Detector=__webpack_require__(13),Constraint=__webpack_require__(8),Composite=__webpack_require__(5),Common=__webpack_require__(0),Bounds=__webpack_require__(1);(function(){MouseConstraint.create=function(engine,options){var mouse=(engine?engine.mouse:null)||(options?options.mouse:
null);mouse||(engine&&engine.render&&engine.render.canvas?mouse=Mouse.create(engine.render.canvas):options&&options.element?mouse=Mouse.create(options.element):(mouse=Mouse.create(),Common.warn("MouseConstraint.create: options.mouse was undefined, options.element was undefined, may not function as expected")));var constraint=Constraint.create({label:"Mouse Constraint",pointA:mouse.position,pointB:{x:0,y:0},length:.01,stiffness:.1,angularStiffness:1,render:{strokeStyle:"#90EE90",lineWidth:3}}),mouseConstraint=
Common.extend({type:"mouseConstraint",mouse,element:null,body:null,constraint,collisionFilter:{category:1,mask:4294967295,group:0}},options);Events.on(engine,"beforeUpdate",function(){var allBodies=Composite.allBodies(engine.world);MouseConstraint.update(mouseConstraint,allBodies);MouseConstraint._triggerEvents(mouseConstraint)});return mouseConstraint};MouseConstraint.update=function(mouseConstraint,bodies){var mouse=mouseConstraint.mouse,constraint=mouseConstraint.constraint,body=mouseConstraint.body;
if(0===mouse.button)if(constraint.bodyB)Sleeping.set(constraint.bodyB,!1),constraint.pointA=mouse.position;else for(var i=0;i<bodies.length;i++){if(body=bodies[i],Bounds.contains(body.bounds,mouse.position)&&Detector.canCollide(body.collisionFilter,mouseConstraint.collisionFilter))for(var j=1<body.parts.length?1:0;j<body.parts.length;j++)if(Vertices.contains(body.parts[j].vertices,mouse.position)){constraint.pointA=mouse.position;constraint.bodyB=mouseConstraint.body=body;constraint.pointB={x:mouse.position.x-
body.position.x,y:mouse.position.y-body.position.y};constraint.angleB=body.angle;Sleeping.set(body,!1);Events.trigger(mouseConstraint,"startdrag",{mouse,body});break}}else constraint.bodyB=mouseConstraint.body=null,constraint.pointB=null,body&&Events.trigger(mouseConstraint,"enddrag",{mouse,body})};MouseConstraint._triggerEvents=function(mouseConstraint){var mouse=mouseConstraint.mouse,mouseEvents=mouse.sourceEvents;mouseEvents.mousemove&&Events.trigger(mouseConstraint,"mousemove",{mouse});mouseEvents.mousedown&&
Events.trigger(mouseConstraint,"mousedown",{mouse});mouseEvents.mouseup&&Events.trigger(mouseConstraint,"mouseup",{mouse});Mouse.clearSourceEvents(mouse)}})()},function(module,exports,__webpack_require__){var Query={};module.exports=Query;var Vector=__webpack_require__(2),SAT=__webpack_require__(14),Bounds=__webpack_require__(1),Bodies=__webpack_require__(11),Vertices=__webpack_require__(3);(function(){Query.collides=function(body,bodies){for(var collisions=[],i=0;i<bodies.length;i++){var bodyA=bodies[i];
if(Bounds.overlaps(bodyA.bounds,body.bounds))for(var j=1===bodyA.parts.length?0:1;j<bodyA.parts.length;j++){var part=bodyA.parts[j];if(Bounds.overlaps(part.bounds,body.bounds)&&(part=SAT.collides(part,body),part.collided)){collisions.push(part);break}}}return collisions};Query.ray=function(bodies,startPoint,endPoint,rayWidth){rayWidth=rayWidth||1E-100;var rayAngle=Vector.angle(startPoint,endPoint),rayLength=Vector.magnitude(Vector.sub(startPoint,endPoint));startPoint=Bodies.rectangle(.5*(endPoint.x+
startPoint.x),.5*(endPoint.y+startPoint.y),rayLength,rayWidth,{angle:rayAngle});bodies=Query.collides(startPoint,bodies);for(startPoint=0;startPoint<bodies.length;startPoint+=1)endPoint=bodies[startPoint],endPoint.body=endPoint.bodyB=endPoint.bodyA;return bodies};Query.region=function(bodies,bounds,outside){for(var result=[],i=0;i<bodies.length;i++){var body=bodies[i],overlaps=Bounds.overlaps(body.bounds,bounds);(overlaps&&!outside||!overlaps&&outside)&&result.push(body)}return result};Query.point=
function(bodies,point){for(var result=[],i=0;i<bodies.length;i++){var body=bodies[i];if(Bounds.contains(body.bounds,point))for(var j=1===body.parts.length?0:1;j<body.parts.length;j++){var part=body.parts[j];if(Bounds.contains(part.bounds,point)&&Vertices.contains(part.vertices,point)){result.push(body);break}}}return result}})()},function(module,exports,__webpack_require__){var Runner={};module.exports=Runner;var Events=__webpack_require__(4),Engine=__webpack_require__(18),Common=__webpack_require__(0);
(function(){if("undefined"!==typeof window){var _requestAnimationFrame=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.msRequestAnimationFrame;var _cancelAnimationFrame=window.cancelAnimationFrame||window.mozCancelAnimationFrame||window.webkitCancelAnimationFrame||window.msCancelAnimationFrame}if(!_requestAnimationFrame){var _frameTimeout;_requestAnimationFrame=function(callback){_frameTimeout=setTimeout(function(){callback(Common.now())},
1E3/60)};_cancelAnimationFrame=function(){clearTimeout(_frameTimeout)}}Runner.create=function(options){options=Common.extend({fps:60,correction:1,deltaSampleSize:60,counterTimestamp:0,frameCounter:0,deltaHistory:[],timePrev:null,timeScalePrev:1,frameRequestId:null,isFixed:!1,enabled:!0},options);options.delta=options.delta||1E3/options.fps;options.deltaMin=options.deltaMin||1E3/options.fps;options.deltaMax=options.deltaMax||1E3/(.5*options.fps);options.fps=1E3/options.delta;return options};Runner.run=
function(runner,engine){"undefined"!==typeof runner.positionIterations&&(engine=runner,runner=Runner.create());(function render(time){runner.frameRequestId=_requestAnimationFrame(render);time&&runner.enabled&&Runner.tick(runner,engine,time)})();return runner};Runner.tick=function(runner,engine,time){var timing=engine.timing,correction=1,event={timestamp:timing.timestamp};Events.trigger(runner,"beforeTick",event);if(runner.isFixed)var delta=runner.delta;else delta=time-runner.timePrev||runner.delta,
runner.timePrev=time,runner.deltaHistory.push(delta),runner.deltaHistory=runner.deltaHistory.slice(-runner.deltaSampleSize),delta=Math.min.apply(null,runner.deltaHistory),delta=delta<runner.deltaMin?runner.deltaMin:delta,delta=delta>runner.deltaMax?runner.deltaMax:delta,correction=delta/runner.delta,runner.delta=delta;0!==runner.timeScalePrev&&(correction*=timing.timeScale/runner.timeScalePrev);0===timing.timeScale&&(correction=0);runner.timeScalePrev=timing.timeScale;runner.correction=correction;
runner.frameCounter+=1;1E3<=time-runner.counterTimestamp&&(runner.fps=(time-runner.counterTimestamp)/1E3*runner.frameCounter,runner.counterTimestamp=time,runner.frameCounter=0);Events.trigger(runner,"tick",event);Events.trigger(runner,"beforeUpdate",event);Engine.update(engine,delta,correction);Events.trigger(runner,"afterUpdate",event);Events.trigger(runner,"afterTick",event)};Runner.stop=function(runner){_cancelAnimationFrame(runner.frameRequestId)};Runner.start=function(runner,engine){Runner.run(runner,
engine)}})()},function(module,exports,__webpack_require__){var Svg={};module.exports=Svg;__webpack_require__(1);var Common=__webpack_require__(0);(function(){Svg.pathToVertices=function(path,sampleLength){"undefined"===typeof window||"SVGPathSeg"in window||Common.warn("Svg.pathToVertices: SVGPathSeg not defined, a polyfill is required.");var i,segmentsQueue,lastPoint,points=[],lx,ly,length=0,x=0,y=0;sampleLength=sampleLength||15;var addPoint=function(px,py,pathSegType){pathSegType=1===pathSegType%
2&&1<pathSegType;if(!lastPoint||px!=lastPoint.x||py!=lastPoint.y){lastPoint&&pathSegType?(lx=lastPoint.x,ly=lastPoint.y):ly=lx=0;var point={x:lx+px,y:ly+py};if(pathSegType||!lastPoint)lastPoint=point;points.push(point);x=lx+px;y=ly+py}},addSegmentPoint=function(segment){var segType=segment.pathSegTypeAsLetter.toUpperCase();if("Z"!==segType){switch(segType){case "M":case "L":case "T":case "C":case "S":case "Q":x=segment.x;y=segment.y;break;case "H":x=segment.x;break;case "V":y=segment.y}addPoint(x,
y,segment.pathSegType)}};Svg._svgPathToAbsolute(path);var total=path.getTotalLength();var segments=[];for(i=0;i<path.pathSegList.numberOfItems;i+=1)segments.push(path.pathSegList.getItem(i));for(segmentsQueue=segments.concat();length<total;){i=path.getPathSegAtLength(length);i=segments[i];if(i!=lastSegment){for(;segmentsQueue.length&&segmentsQueue[0]!=i;)addSegmentPoint(segmentsQueue.shift());var lastSegment=i}switch(i.pathSegTypeAsLetter.toUpperCase()){case "C":case "T":case "S":case "Q":case "A":i=
